...
describe("ShoppingSessionService - endSession", () => {
  ...
  it("should end a shopping session successfully", async () => {
    ...
    expect(service["sessionRepository"].findById).toHaveBeenCalledWith(
      mockSessionId,
    );
    expect(service["sessionRepository"].getSessionLists).toHaveBeenCalledWith(
      mockSessionId,
    );
    expect(service["listRepository"].unlockList).toHaveBeenCalledTimes(2);
    expect(service["listRepository"].unlockList).toHaveBeenCalledWith(
      mockListId1,
    );
    expect(service["listRepository"].unlockList).toHaveBeenCalledWith(
      mockListId2,
  ...
  it("should create a new list for unpurchased items when requested", async () => {
    ...
    expect(service["itemRepository"].findByList).toHaveBeenCalledWith(
      mockListId1,
    );
    expect(service["itemRepository"].findByList).toHaveBeenCalledWith(
      mockListId2,
  ...
  it("should handle database errors gracefully", async () => {
    ...
    // Mock database error
    (
      service["sessionRepository"].endSession as jest.Mock
