...
describe("ShoppingSessionService - createSession", () => {
  ...
  it("should create a shopping session successfully", async () => {
    ...
    expect(service["sessionRepository"].findActiveByUser).toHaveBeenCalledWith(
      mockUserId,
    );
    expect(service["listRepository"].findById).toHaveBeenCalledWith(
      mockListId1,
    );
    expect(service["listRepository"].findById).toHaveBeenCalledWith(
      mockListId2,
    ...
    expect(service["sessionRepository"].addListToSession).toHaveBeenCalledTimes(
      2,
    );
    expect(service["listRepository"].lockList).toHaveBeenCalledTimes(2);
    expect(service["listRepository"].lockList).toHaveBeenCalledWith(
      mockListId1,
    );
    expect(service["listRepository"].lockList).toHaveBeenCalledWith(
      mockListId2,
  ...
  it("should return error if user already has an active session", async () => {
    ...
    (
      service["sessionRepository"].findActiveByUser as jest.Mock
    ...
    expect(
      service["sessionRepository"].addListToSession,
  ...
  it("should return error if a list is not found", async () => {
    ...
    // Mock list not found
    (service["listRepository"].findById as jest.Mock).mockImplementation(
      (id) => {
        if (id === mockListId1) {
          return Promise.resolve({
            id: mockListId1,
  ...
  it("should handle database errors gracefully", async () => {
    ...
    // Mock database error
    (service["sessionRepository"].save as jest.Mock).mockRejectedValueOnce(
