import { ShoppingSession, ShoppingSessionStatus, ShoppingList } from '../types/models';
import { CreateShoppingSessionParams, EndShoppingSessionParams, Result } from '../types/operations';
import { ShoppingSessionRepository, DexieShoppingSessionRepository } from '../repositories/shopping-session-repository';
import { ShoppingListRepository, DexieShoppingListRepository } from '../repositories/shopping-list-repository';
import { ListItemRepository, DexieListItemRepository } from '../repositories/list-item-repository';
...
export interface ShoppingSessionService {
  createSession(params: CreateShoppingSessionParams): Promise<Result<ShoppingSession>>;
  getActiveSession(userId: string): Promise<Result<ShoppingSession | null>>;
  getSessionWithLists(sessionId: string): Promise<Result<{ session: ShoppingSession, lists: ShoppingList[] }>>;
  addListToSession(sessionId: string, listId: string): Promise<Result<void>>;
  removeListFromSession(sessionId: string, listId: string): Promise<Result<void>>;
  endSession(params: EndShoppingSessionParams): Promise<Result<ShoppingSession>>;
...
export class LocalShoppingSessionService implements ShoppingSessionService {
   ...
   */
  async createSession(params: CreateShoppingSessionParams): Promise<Result<ShoppingSession>> {
    try {
      // Check if the user already has an active session
      const existingSession = await this.sessionRepository.findActiveByUser(params.userId);
      if (existingSession) {
        return { success: false, error: 'User already has an active shopping session' };
      }
      ...
      for (const listId of params.listIds) {
        ...
        if (list.isLocked) {
          return { success: false, error: `List with ID ${listId} is already locked` };
        }
      }
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'Failed to create shopping session' 
      };
    }
  }
   ...
   */
  async getActiveSession(userId: string): Promise<Result<ShoppingSession | null>> {
    try {
   ...
   */
  async getSessionWithLists(sessionId: string): Promise<Result<{ session: ShoppingSession, lists: ShoppingList[] }>> {
    try {
      const { session, lists: listIds } = await this.sessionRepository.findWithLists(sessionId);
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'Failed to get session with lists' 
      };
    }
  }
   ...
   */
  async addListToSession(sessionId: string, listId: string): Promise<Result<void>> {
    try {
      ...
      if (session.status !== ShoppingSessionStatus.ACTIVE) {
        return { success: false, error: 'Cannot add lists to a non-active session' };
      }
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'Failed to add list to session' 
      };
    }
  }
   ...
   * @returns Result indicating success or failure
   */
  async removeListFromSession(sessionId: string, listId: string): Promise<Result<void>> {
    try {
      // Check if the session exists and is active
      const session = await this.sessionRepository.findById(sessionId);
      if (!session) {
      ...
      if (session.status !== ShoppingSessionStatus.ACTIVE) {
        return { success: false, error: 'Cannot remove lists from a non-active session' };
      }
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'Failed to remove list from session' 
      };
    }
  }
   ...
   */
  async endSession(params: EndShoppingSessionParams): Promise<Result<ShoppingSession>> {
    try {
      // Check if the session exists and is active
      ...
      const listIds = await this.sessionRepository.getSessionLists(params.sessionId);
      ...
      // End the session
      const updatedSession = await this.sessionRepository.endSession(params.sessionId, params.status);
      
      return { success: true, data: updatedSession };
    } catch (error) {
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'Failed to end shopping session' 
      };
    }
  }
  ...
  async getItemsBySourceList(sessionId: string): Promise<Result<any>> {
    try {
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'Failed to get items by source list' 
      };
    }
  }
