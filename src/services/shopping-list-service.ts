...
import { CreateListParams, UpdateListParams, CreateListItemParams, UpdateListItemParams, Result, ShareListParams } from '../types/operations';
...
import { isValidListName, isNotEmpty, isValidQuantity, isValidUnit } from '../utils/validation/validators';

/**
 * Service interface for shopping list operations
 */
export interface ShoppingListService {
  createList(params: CreateListParams, userId: string): Promise<Result<ShoppingList>>;
  updateList(params: UpdateListParams, userId: string): Promise<Result<ShoppingList>>;
...
export class LocalShoppingListService implements ShoppingListService {
   ...
   */
  async createList(params: CreateListParams, userId: string): Promise<Result<ShoppingList>> {
    try {
      // Validate input
   ...
   */
  async updateList(params: UpdateListParams, userId: string): Promise<Result<ShoppingList>> {
    try {
      ...
      if (!isOwner) {
        return { success: false, error: 'You do not have permission to update this list' };
      }
      ...
      if (params.communityId !== undefined) updates.communityId = params.communityId;
  ...
  async deleteList(id: string, userId: string): Promise<Result<void>> {
    try {
      ...
      if (!isOwner) {
        return { success: false, error: 'You do not have permission to delete this list' };
      }
  ...
  async getUserLists(userId: string): Promise<Result<ShoppingList[]>> {
    try {
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'Failed to get user lists' 
      };
    }
  }
   ...
   * @returns Result with an array of lists
   */
  async getCommunityLists(communityId: string): Promise<Result<ShoppingList[]>> {
    try {
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'Failed to get community lists' 
      };
    }
  }
   ...
   */
  async updateListItem(params: UpdateListItemParams): Promise<Result<ListItem>> {
    try {
      ...
      if (list?.isLocked && params.isPurchased === undefined) {
        return { success: false, error: 'Cannot update items in a locked list except for purchase status' };
      }
  ...
  async removeItemFromList(itemId: string): Promise<Result<void>> {
    try {
      ...
      const list = await this.listRepository.findById(item.listId);
      if (list?.isLocked) {
        return { success: false, error: 'Cannot remove items from a locked list' };
      }
  ...
  async shareList(params: ShareListParams): Promise<Result<void>> {
    try {
      ...
      const isAlreadyOwner = await this.isListOwner(params.listId, params.userId);
      if (isAlreadyOwner) {
        return { success: false, error: 'User is already an owner of this list' };
      }
  ...
  async unshareList(listId: string, userId: string): Promise<Result<void>> {
    try {
      ...
      if (list.createdBy === userId) {
        return { success: false, error: 'Cannot remove the creator of the list' };
      }
  ...
  async getListOwners(listId: string): Promise<Result<string[]>> {
    try {
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'Failed to get list owners' 
      };
    }
  }
